...
> ⚠️ **不要跳过这一步。** 即使一次打卡只有一个 stop，也必须先把 day 容
> 器建好，再把 stop 的 `day` 字段指向它。Step 3b 的 `day` 字段绝不能空。

```bash
TODAY=$(date +%Y-%m-%d)

# 关键：PB 的 date 字段实际存 "YYYY-MM-DD HH:MM:SS.sssZ"，所以
# 用 `date='$TODAY'` 等值比较永远 0 命中（→ 又建一个 day → 重复）。
# 必须用 `date~'$TODAY'` 模糊匹配（LIKE 前缀），才能命中今天的行。
...
- **永远用 `date~` 不用 `date=`**，因为 PB date 字段带时分秒，等值比不上

**严格命名规则**（绝不允许花样翻新）：

| 场景 | 强制格式 | 例子 |
|---|---|---|
| 散落打卡（trip 空）+ 有 `location.city` | `<City> <YYYY-MM-DD>` | `Levittown 2026-06-04` |
| 散落打卡（trip 空）+ 没 `location.city`（或 build_location=false） | `<YYYY-MM-DD>`（只日期，无后缀） | `2026-06-15` |
| 行程打卡（trip 命中）+ trip day 已存在 | **不动**，复用现有 name | `台北 Day 2` |
| 行程打卡（trip 命中）+ trip day 不存在（罕见兜底） | `<trip.title> <YYYY-MM-DD>` | `日本关西 2026-06-04` |

**禁止的变体（任何场景都不准）：**
- `2026-06-04 Levittown`（顺序反）
- `周四 Levittown` / `Levittown 周四 2026-06-04`（加星期）
- `Levittown · 2026-06-04` / `Levittown - 2026-06-04`（加分隔符）
- `打卡 Levittown 2026-06-04` / `Day Levittown 2026-06-04`（加前缀）
- `Levittown` 或 `2026/06/04` 或 `06-04`（缺日期或格式不 ISO）
- 任何全角字符、emoji、中文标点
...
7. **Step 3a 不可跳过**：每个 Stop 必须有 `day` 字段指向一个 Day 行。Day 不存在就 upsert，绝不让 `stop.day` 留空。同一天同 trip 只该有一个 Day；用 `date~'$TODAY'` 做模糊匹配（不要 `date='$TODAY'`，PB date 字段带时分秒等值比不上）。
8. **Day name 严格按格式**：散落打卡用 `<City> <YYYY-MM-DD>`（无 city 退化为 `<YYYY-MM-DD>`），行程打卡用 `<trip.title> <YYYY-MM-DD>`（或复用 trip 行程表里已建好的 day name）。绝不发明其他写法（无星期、无分隔符、无 emoji、不调换顺序）。详见 Step 3a 命名表。
